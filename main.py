...
for filepath in glob.glob('SAUS/*.csv'):
    with open(os.path.join(os.getcwd(), filepath), 'r', encoding='cp1251') as file:
        ...
        for i in table.index:
            if str(table['name'][i]) == filename[:-4]:
                R+=1
        ...
        print(f"F1 = {f1:.3f}")
        # Создаем строку для записи в DataFrame
        row = {'File': os.path.basename(filepath), 'Precision': precision, 'Recall': recall, 'F1': f1}
...
# Создание DataFrame из списка результатов и сохранение в Excel-файл
df = pd.DataFrame(results)
df.to_excel('results.xlsx',sheet_name='Набор данных SAUS', index=False)